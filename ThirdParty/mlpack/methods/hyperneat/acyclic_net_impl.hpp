...
#ifndef MLPACK_METHODS_HNEAT_ACYCLIC_NET_IMPL_HPP
#define MLPACK_METHODS_HNEAT_ACYCLIC_NET_IMPL_HPP
                                                  ...
                                                  nodeDepths)
{
  ...
  for (size_t i = 0; i < layers.size(); i++)
  {
    for (size_t j = 0; j < layers[i].size(); j++)
    {
      ...
      {
        double result = MultimodalActivationFunction::Fn(activationGraph[nodeID], nodeValues[nodeID]);
        for (auto const& x : directedGraph[nodeID])
        {
          if (x.second.Enabled())
